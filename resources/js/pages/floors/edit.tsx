...
export default function EditFloor() {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    card.style.transform = `perspective(900px) rotateX(${((e.clientY-top)/height-0.5)*-8}deg) rotateY(${((e.clientX-left)/width-0.5)*8}deg) translateY(-4px)`
  }
  const handleMouseLeave = () => { if (cardRef.current) cardRef.current.style.transform = 'perspective(900px) rotateX(0) rotateY(0) translateY(0)' }
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!Swal) { put(`/floors/${floor.id}`); return }
    if (!String(data.floorNumber).trim()) {
      Swal.fire({ ...swalBase, icon: 'warning', iconColor: '#f59e0b', title: 'Missing Fields', text: 'Floor Number is required.', confirmButtonColor: '#16a34a' }); return
    }
    Swal.fire({ ...swalBase, title: 'Updating Floor…', html: '<span style="color:#4a7c59;font-size:.85rem">Saving changes…</span>', allowOutsideClick: false, didOpen: () => Swal.showLoading() })
    put(`/floors/${floor.id}`, {
      onSuccess: () => Swal.fire({ ...swalBase, icon: 'success', iconColor: '#16a34a', title: '🌿 Floor Updated!',
        html: `<span style="color:#4a7c59">Floor <b style="color:#15803d">${data.name || '#'+data.floorNumber}</b> updated successfully.</span>`,
      ...
      onError: () => Swal.fire({ ...swalBase, icon: 'error', iconColor: '#dc2626', title: 'Update Failed', text: 'Please check the form and try again.', confirmButtonColor: '#16a34a' }),
    })
  }

  function handleCancel() {
    const Swal = (window as any).Swal
    const dirty = String(data.floorNumber) !== String(floor?.floorNumber ?? '') || data.name !== (floor?.name ?? '')
    if (!dirty) { window.history.back(); return }
    Swal?.fire({ ...swalBase, icon: 'question', iconColor: '#0ea5e9', title: 'Discard Changes?', text: 'Your edits will be lost.',
  ...
  if (!floor) return (
    <div style={{ fontFamily: 'Outfit,sans-serif', minHeight: '100vh', background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#86a898' }}>
      Loading floor…
    </div>
  )
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* amber edit ribbon */
        .cb-edit-ribbon{position:absolute;top:18px;right:-28px;background:linear-gradient(135deg,#f59e0b,#fbbf24);color:#7c2d12;font-family:'JetBrains Mono',monospace;font-size:.6rem;font-weight:500;letter-spacing:.1em;text-transform:uppercase;padding:4px 40px;transform:rotate(45deg);box-shadow:0 2px 8px rgba(245,158,11,.3);z-index:2}
        ...
        .cb-prog-pct{font-family:'JetBrains Mono',monospace;font-size:.7rem;font-weight:500;color:var(--green)}
        .cb-prog-track{height:5px;background:var(--green-pale);border-radius:100px;overflow:hidden}
        .cb-prog-fill{height:100%;border-radius:100px;background:linear-gradient(90deg,#f59e0b,var(--green),var(--teal-lt));box-shadow:0 0 10px var(--green-glow);transition:width .55s cubic-bezier(.22,1,.36,1)}
        .cb-header{margin-bottom:1.6rem;animation:fadeUp .5s .12s both}
        .cb-badge{display:inline-flex;align-items:center;gap:6px;background:#fffbeb;border:1px solid rgba(245,158,11,.3);border-radius:100px;padding:3px 12px 3px 8px;margin-bottom:1rem;font-size:.7rem;font-weight:500;color:#92400e;letter-spacing:.02em}
        .cb-badge-dot{width:7px;height:7px;border-radius:50%;background:#f59e0b;box-shadow:0 0 6px rgba(245,158,11,.4);animation:dotPulse 2s ease-in-out infinite}
        @keyframes dotPulse{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(.7);opacity:.5}}
        .cb-floor-id{display:inline-flex;align-items:center;gap:5px;font-family:'JetBrains Mono',monospace;font-size:.65rem;color:var(--muted);background:var(--green-pale);border:1px solid rgba(22,163,74,.2);border-radius:6px;padding:2px 9px;margin-left:8px}
        ...
        .cb-title-accent{background:linear-gradient(135deg,#f59e0b 0%,var(--green) 50%,var(--teal) 100%);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:shimmer 4s linear infinite}
        ...
        .cb-input::placeholder{color:#a8c4b0}.cb-input:hover{border-color:rgba(22,163,74,.35);background:#f0fdf4}
          ...
          <div className="cb-body">
            ...
            <div className="cb-header">
              <div className="cb-badge">
                <span className="cb-badge-dot"/>Edit Mode
                <span className="cb-floor-id">Floor #{floor.id}</span>
              </div>
              <div className="cb-eyebrow"><span className="cb-ey-line"/>Floor Registry<span className="cb-ey-line"/></div>
              <h1 className="cb-title">Edit <span className="cb-title-accent">Floor</span></h1>
              <p className="cb-sub">Update details for <strong style={{color:'var(--green-deep)'}}>{floor.name || `Floor ${floor.floorNumber}`}</strong>.</p>
            </div>
            ...
            <form onSubmit={submit}>
              <div className="cb-field">
                <div className="cb-label-row">
                  <label className={`cb-label${focused==='floorNumber'?' act':''}`}>
                    <span className="cb-licon"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 21h18M3 7h18M3 14h18M4 3h16a1 1 0 0 1 1 1v17H3V4a1 1 0 0 1 1-1z"/></svg></span>
                    Floor Number
                  </label>
                  <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.6rem',color:'var(--muted)'}}>Required</span>
                </div>
                <div className="cb-input-wrap">
                  <input type="number" className="cb-input" value={data.floorNumber}
                  ...
                  />
                  <span className={`cb-check${String(data.floorNumber).trim()?' vis':''}`}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8"><polyline points="20 6 9 17 4 12"/></svg></span>
              ...
              <div className="cb-field">
                <div className="cb-label-row">
                  <label className={`cb-label${focused==='name'?' act':''}`}>
                    <span className="cb-licon"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/></svg></span>
                    Floor Name
                  </label>
                  <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.6rem',color:'var(--muted)'}}>Optional</span>
                </div>
                <div className="cb-input-wrap">
                  <input className="cb-input" placeholder="e.g. Ground Floor, Lobby…" value={data.name}
                    onChange={(e) => setData('name', e.target.value)}
                    onFocus={() => setFocused('name')} onBlur={() => setFocused(null)}
                    style={{ paddingRight: data.name.trim() ? '2.4rem' : undefined }}
                  />
                  <span className={`cb-check${data.name.trim()?' vis':''}`}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8"><polyline points="20 6 9 17 4 12"/></svg></span>
                  <div className="cb-glow-line"/>
                </div>
              ...
              <div className="cb-footer">
                <button type="button" className="cb-cancel" onClick={handleCancel}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>Cancel
                </button>
                <button type="submit" className="cb-submit" disabled={processing}>
                  {processing ? <span className="cb-spin"/> : <>Save Changes<span className="cb-arr"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg></span></>}
                </button>
              </div>
            </form>

            <div className="cb-infobar">
              <span>✏️ SWACHH CAMPUS</span><span className="cb-dot"/><span>EDIT FLOOR</span><span className="cb-dot"/><span>{new Date().getFullYear()}</span>
            </div>
          </div>
        </div>
      </div>
